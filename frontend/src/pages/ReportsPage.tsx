...
import {
  ...
  BarChart3, ArrowUpRight, ArrowDownRight, Sparkles, Loader2,
} from 'lucide-react';
...
export default function ReportsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        if (tab === 'pnl') setPnl(await api.getPLReport(months));
        else if (tab === 'expenses') setExpenses(await api.getExpenseReport(months));
        else if (tab === 'cashflow') setCashflow(await api.getCashFlowReport(months));
        else if (tab === 'summary') setSummary(await api.getBusinessSummary());
      } catch { /* silent */ }
